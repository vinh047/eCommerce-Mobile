    ...
    <div className="fixed inset-0 z-50 overflow-hidden" onClick={(e) => e.target === e.currentTarget && onClose()}>
      ...
      <div className="absolute top-0 right-0 h-full w-full max-w-md bg-white dark:bg-gray-800 shadow-2xl transform transition-transform duration-300 flex flex-col">
         <div className="p-6 bg-yellow-500 text-white">
            ...
            <div className="mt-4">
               <div className="flex items-center space-x-1">
                  {[...Array(5)].map((_, i) => (
                     <Star key={i} className={`w-5 h-5 ${i < review.stars ? "fill-white text-white" : "text-yellow-200"}`} />
                  ))}
               </div>
               <p className="text-yellow-100 text-sm mt-1">Ngày: {new Date(review.createdAt).toLocaleDateString('vi-VN')}</p>
            </div>
         </div>
         ...
         <div className="flex-1 p-6 overflow-y-auto space-y-6">
            <div className="flex items-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
               ...
               <div>
                  ...
                  <p className="text-xs text-gray-500">về sản phẩm <span className="font-semibold text-blue-600">{review.product?.name}</span></p>
               </div>
            </div>
            ...
            {photos.length > 0 && (
               <div>
                  <h4 className="font-semibold mb-2 dark:text-white">Hình ảnh ({photos.length}):</h4>
                  <div className="grid grid-cols-3 gap-2">
                     {photos.map((url, i) => (
                        <img key={i} src={url} alt="review-img" className="w-full h-24 object-cover rounded-lg border dark:border-gray-600" />
                     ))}
                  </div>
               </div>
            )}
         ...
         <div className="p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
             <button onClick={onEdit} className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium shadow">
