...
type ReqBody = {
  ...
  address?: { province?: string | null; district?: string | null; ward?: string | null } | null;
};
...
export async function POST(req: Request) {
  try {
    ...
    if (subtotal !== null && subtotal !== undefined) {
      ...
      sub = items.reduce((s: number, it: any) => s + (Number(it?.price || 0) * Number(it?.quantity || 0)), 0);
    }

    // basic address shape check (optional)
    const addr = address && typeof address === "object" ? address : null;

    const shippingFee = calcShippingSimple(sub, addr);

    // if calcShippingSimple returns -1 => not deliverable
    if (shippingFee === -1) {
      return NextResponse.json(
        { shippingFee: null, deliverable: false, message: "Không hỗ trợ giao tới địa chỉ này" },
        { status: 200 }
      );
    }

    return NextResponse.json({ shippingFee, deliverable: true }, { status: 200 });
  } catch (err: any) {
