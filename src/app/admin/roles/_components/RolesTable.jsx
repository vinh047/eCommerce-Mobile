...
function RolesTableRow({ role, onEdit, onDelete }) {
  ...
  return (
    <tr className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
      <td className="px-6 py-4 font-mono text-sm text-gray-900 dark:text-white">#{role.id}</td>
      ...
      <td className="px-6 py-4">
        <div className="flex space-x-2">
          <button onClick={() => onEdit(role)} className="p-1 text-blue-600 hover:bg-blue-50 rounded dark:hover:bg-gray-600" title="Sửa">
            <Edit className="w-4 h-4" />
          </button>
          <button onClick={() => onDelete(role.id)} className="p-1 text-red-600 hover:bg-red-50 rounded dark:hover:bg-gray-600" title="Xóa">
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Danh sách Vai trò & Quyền hạn</h3>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {roles.length === 0 ? (
               <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">Chưa có vai trò nào.</td></tr>
            ) : (
              roles.map(role => (
