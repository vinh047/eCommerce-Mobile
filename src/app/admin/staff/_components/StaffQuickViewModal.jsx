             ...
             <div className="space-y-4">
                ...
                <div>
                   <h4 className="flex items-center font-semibold mb-3 text-gray-900 dark:text-white">
                      <Shield className="w-4 h-4 mr-2 text-blue-500" /> Vai trò & Quyền hạn
                   </h4>
                   <div className="flex flex-wrap gap-2">
                      {staff.staffRoles?.length > 0 ? (
                         staff.staffRoles.map(sr => (
                            <span key={sr.roleId} className="px-3 py-1 bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200 rounded-full text-sm">
                               {sr.role?.name || `Role ${sr.roleId}`}
                            </span>
                         ))
                      ) : (
                         <span className="text-gray-500 text-sm italic">Chưa được gán vai trò nào.</span>
                      )}
                   </div>
                </div>
             </div>
