          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg">
              <div className="flex items-center text-green-700 mb-2">
                <DollarSign className="w-4 h-4 mr-1" /> Giá bán
              </div>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
              <div className="flex items-center text-blue-700 mb-2">
                <Package className="w-4 h-4 mr-1" /> Tồn kho
              </div>
              <div className="text-2xl font-bold text-blue-800 dark:text-blue-300">
                {variant.stock}
              </div>
              <div className="text-xs text-gray-500">
                Ngưỡng thấp: {variant.lowStockThreshold}
              </div>
            </div>
          </div>

          {/* Specifications (Dynamic List) */}
          {variant.variantSpecValues &&
            variant.variantSpecValues.length > 0 && (
              <div>
                ...
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 space-y-2">
                  {variant.variantSpecValues.map((spec, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between text-sm border-b border-gray-200 last:border-0 pb-2 last:pb-0"
                    >
                      <span className="text-gray-500">
                        {spec.spec?.label || "Thuộc tính"}:
                      </span>
